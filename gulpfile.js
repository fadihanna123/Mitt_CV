...
const Workwithjs = () => {
  return src(files.jsPath).pipe(dest("pub/js/")).pipe(browserSync.stream());
};

// Task: Läsa in htmlPath från files, skicka också den till pub/ mappen och gör livereload.
const htmlTask = () => {
  return src(files.htmlPath).pipe(dest("pub/")).pipe(browserSync.stream());
};
...
const MoveImages = () => {
  return src(files.ImagesPath).pipe(dest("pub/Images"));
};
